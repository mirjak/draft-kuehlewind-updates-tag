...
simply describe the changes in text. This document recommends the use of the OLD/NEW style in Amending RFCs for minor and
limited number of changes. This makes the changes unambiguous and specific and also
makes it possible for automated tools to consume these changes and be able to produce a marked up copy of the Amended RFC that shows
the effect of these changes in place. If extensive or large number of changes are needed, a new document revision that 
obsoletes the old RFC might be a better option.
